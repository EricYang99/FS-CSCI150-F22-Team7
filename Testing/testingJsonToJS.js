//This is the class data
let data = [
	{
		"Names": "CSCI 1. Critical Thinking and Computer Science",
		"Units": "Units: 3",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "GE Area: A3\r"
	},
	{
		"Names": "CSCI 5. Computer and Applications",
		"Units": "Units: 3",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 30. Introduction to the Internet",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 39. Computer Science Principles",
		"Units": "Units: 3",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 40. Introduction to Programming and Problem Solving",
		"Units": "Units: 4",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 41. Introduction to Data Structures",
		"Units": "Units: 4",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 60. Foundations of Computer Science",
		"Units": "Units: 4",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 68. Computing Practice and Programming",
		"Units": "Units: 4",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 100. Introduction to Computational Science",
		"Units": "Units: 3",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "GE Area: IB\r"
	},
	{
		"Names": "CSCI 101. Computational Foundations for Bioinformatics",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 108. Algorithmic Thinking",
		"Units": "Units: 4",
		"Usually Offered In": "Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 112. Introduction to Computer Systems",
		"Units": "Units: 4",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 113. Introduction to Computer Organization",
		"Units": "Units: 4",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 114. Introduction to Operating Systems",
		"Units": "Units: 3",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 115. Algorithms and Data Structures",
		"Units": "Units: 4",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 117. Structures of Programming Languages",
		"Units": "Units: 4",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 119. Introduction to Finite Automata",
		"Units": "Units: 4",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 124. Introduction to File Processing",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 126. Database Systems",
		"Units": "Units: 3",
		"Usually Offered In": "Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 130. Web Programming",
		"Units": "Units: 3",
		"Usually Offered In": "Fall",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 134. Compiler Design",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 146. Systems Architecture",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 148. Systems Programming",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 150. Introduction to Software Engineering",
		"Units": "Units: 3",
		"Usually Offered In": "Fall",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 152E. Software Engineering",
		"Units": "Units: 3",
		"Usually Offered In": "Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 154. Simulation",
		"Units": "Units: 3",
		"Usually Offered In": "Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 156. Internetworking Systems and Protocols",
		"Units": "Units: 3",
		"Usually Offered In": "Fall",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 164. Artificial Intelligence Programming",
		"Units": "Units: 3",
		"Usually Offered In": "Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 166. Principles of Artificial Intelligence",
		"Units": "Units: 3",
		"Usually Offered In": "Fall",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 168E. Big Data Analytics",
		"Units": "Units: 3",
		"Usually Offered In": "Fall",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 172. Computer Graphics",
		"Units": "Units: 3",
		"Usually Offered In": "Fall",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 173. Advanced Computer Graphics",
		"Units": "Units: 3",
		"Usually Offered In": "Fall",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 174. Design and Analysis of Algorithms",
		"Units": "Units: 3",
		"Usually Offered In": "Fall",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 176. Parallel Processing",
		"Units": "Units: 3",
		"Usually Offered In": "Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 177. Distributed Computer Systems",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 186. Formal Languages and Automata",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 188. Introduction to Computability",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 190. Independent Study",
		"Units": "Units: 1-3",
		"Usually Offered In": "",
		"Additional Information \r": " Repeatable up to 6 units\r"
	},
	{
		"Names": "CSCI 191T. Topics in Computer Science",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": " Repeatable up to 9 units\r"
	},
	{
		"Names": "CSCI 191T. Computer Security",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 194I. Cooperative Education",
		"Units": "Units: 1-4",
		"Usually Offered In": "",
		"Additional Information \r": " Repeatable up to 8 units\r"
	},
	{
		"Names": "CSCI 198. Project",
		"Units": "Units: 3",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 200. Introduction to Research in Computer Science",
		"Units": "Units: 1",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 201. CSCI Colloquium",
		"Units": "Units: 2",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 213. Computer Organization",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 217. Programming Language Principles",
		"Units": "Units: 3",
		"Usually Offered In": "Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 226. Advanced Database Systems",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 230. Advanced Web Application Development",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 244. Operating Systems",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 246. Computer Architecture",
		"Units": "Units: 3",
		"Usually Offered In": "Fall",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 250. Advanced Software Engineering",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 252. Software Development and Environments",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 253. Human-Computer Interaction",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 256. Wireless Communications and Mobile Computing",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 264. Artificial Intelligence",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 272. Computer Graphics",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 274. Combinatorial Algorithms",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 282. Theory of Computation",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 284. Automata Theory",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 290. Independent Study",
		"Units": "Units: 1-3",
		"Usually Offered In": "",
		"Additional Information \r": " Repeatable up to 6 units\r"
	},
	{
		"Names": "CSCI 291T. Seminar",
		"Units": "Units: 1-3",
		"Usually Offered In": "",
		"Additional Information \r": " Repeatable up to 9 units\r"
	},
	{
		"Names": "CSCI 291T. Introduction to Reinforcement Learning",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 291T. Image Processing",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 297. Grad Synthesis",
		"Units": "Units: 3",
		"Usually Offered In": "Fall / Spring",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 298. Research Project",
		"Units": "Units: 3",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 298C. Project Continuation",
		"Units": "Units: 0",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 299. Master's Thesis",
		"Units": "Units: 3-6",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	},
	{
		"Names": "CSCI 299C. Thesis Continuation",
		"Units": "Units: 0",
		"Usually Offered In": "",
		"Additional Information \r": "\r"
	}
];
//Creating Arrays to hold all the data
let nameArr = [];
let unitArr = [];
let offerArr = [];
let additionalInfoArr = [];
for(let i = 0; i < data.length; i++){ //We can do either sytle -> .name or ["name"]
	nameArr.push(data[i]["Names"]);
	unitArr.push(data[i]["Units"]);
	offerArr.push(data[i]["Usually Offered In"]);
	additionalInfoArr.push(data[i]["Additional Information \r"]);
};

function showClassData(){
	let docBod = document.getElementById("ClassContainer");
	let newTable= document.createElement('TABLE');
	let newTableBod = document.createElement('TBODY');
	docBod.appendChild(newTable);
	newTable.appendChild(newTableBod);

    for(let i = 0; i < data.length; i++){
		let tr = document.createElement('TR');
		newTableBod.appendChild(tr);
		for(let j = 0; j < 4 && i < data.length; j++, i++){
			if(data[i]["Names"] == ""){
			}
			else{
				let classInfo = createTd(data[i]["Names"], data[i]["Units"], data[i]["Usually Offered In"], data[i]["Additional Information \r"]);
				tr.appendChild(classInfo);
			}
			if(j == 3){
				i--;
			}
        }
        
    }
}

function createTd(dataOne, dataTwo, dataThree, dataFour){

	let dataList = document.createElement('TD')

	let className = document.createElement('H3');
	className.appendChild(document.createTextNode(dataOne));

	let unit = document.createElement('P');
	unit.appendChild(document.createTextNode(dataTwo));

	if(!dataThree && (!dataFour || dataFour == "\r")){
		dataList.appendChild(className);
		dataList.appendChild(unit);
	}
	else if(!dataThree && dataFour){
		let moreInfo = document.createElement('P');
		moreInfo.appendChild(document.createTextNode(dataFour));

		dataList.appendChild(className);
		dataList.appendChild(unit);
		dataList.appendChild(moreInfo);
	}
	else if(dataFour == "\r" && dataThree){
		let offered = document.createElement('P');
		offered.appendChild(document.createTextNode(dataThree));

		dataList.appendChild(className);
		dataList.appendChild(unit);
		dataList.appendChild(offered);
	}
	else{
		let offered = document.createElement('P');
		offered.appendChild(document.createTextNode(dataThree));
		let moreInfo = document.createElement('P');
		moreInfo.appendChild(document.createTextNode(dataFour));

		dataList.appendChild(className);
		dataList.appendChild(unit);
		dataList.appendChild(offered);
		dataList.appendChild(moreInfo);
	}
	return dataList;
}

//Return all the class data
function getClassInfo(){
	return data;
}
//Return a single instance of each class
function getClassName(index){
	return nameArr[index];
}
function getClassUnits(index){
	return unitArr[index];
}
function getClassOffered(index){
	return offerArr[index];
}
function getClassAdditionalInfo(index){
	return additionalInfoArr[index];
}
//Return all the info on one class
function getOneClass(index){
	let singleClass = [getClassName(index), getClassUnits(index), getClassOffered(index), getClassAdditionalInfo(index)];
	return singleClass;
}

function createDivBody(index){
	let singleClass = getOneClass(index);

	let dataList = document.createElement('div')

	let className = document.createElement('H3');
	className.appendChild(document.createTextNode(singleClass[0]));

	let unit = document.createElement('P');
	unit.appendChild(document.createTextNode(singleClass[1]));

	if(!singleClass[2] && (!singleClass[3] || singleClass[3] == "\r")){
		dataList.appendChild(className);
		dataList.appendChild(unit);
	}
	else if(!singleClass[2] && singleClass[3]){
		let moreInfo = document.createElement('P');
		moreInfo.appendChild(document.createTextNode(singleClass[3]));

		dataList.appendChild(className);
		dataList.appendChild(unit);
		dataList.appendChild(moreInfo);
	}
	else if(singleClass[3] == "\r" && singleClass[2]){
		let offered = document.createElement('P');
		offered.appendChild(document.createTextNode(singleClass[2]));

		dataList.appendChild(className);
		dataList.appendChild(unit);
		dataList.appendChild(offered);
	}
	else{
		let offered = document.createElement('P');
		offered.appendChild(document.createTextNode(singleClass[2]));
		let moreInfo = document.createElement('P');
		moreInfo.appendChild(document.createTextNode(singleClass[3]));

		dataList.appendChild(className);
		dataList.appendChild(unit);
		dataList.appendChild(offered);
		dataList.appendChild(moreInfo);
	}
	return dataList;
}



//This is where the professor's data is
let profData = [
	{
		"Name": "Dhanyu Eshaka Amarasinghe",
		"Faculty Role": "Part Time Faculty",
		"Desk Phone Number": "559-278-3026",
		"Department Phone Number": "559-278-4373",
		"Email Address": "dhanyu@csufresno.edu",
		"Office Location": "Science II 249",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Dr. Brent Auernheimer",
		"Faculty Role": "Faculty",
		"Desk Phone Number": "559-278-2573",
		"Department Phone Number": "559-278-4373",
		"Email Address": "brent_auernheimer@csufresno.edu",
		"Office Location": "Science II 280",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Santanu Banerjee",
		"Faculty Role": "Part Time Faculty",
		"Desk Phone Number": "559-278-3026",
		"Department Phone Number": "559-278-4373",
		"Email Address": "santanub@csufresno.edu",
		"Office Location": "Science II 249",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Amith Kamath Belman",
		"Faculty Role": "Faculty",
		"Desk Phone Number": "559-278-7074",
		"Department Phone Number": "559-278-4373",
		"Email Address": "akamathbelman@mail.fresnostate.edu",
		"Office Location": "Science II 253",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Hubert Cecotti",
		"Faculty Role": "Faculty",
		"Desk Phone Number": "559-278-2905",
		"Department Phone Number": "559-278-4373",
		"Email Address": "hcecotti@csufresno.edu",
		"Office Location": "Science II 263",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Lisa Dennis",
		"Faculty Role": "Staff / Administrative Asstant",
		"Desk Phone Number": "559-278-4156",
		"Department Phone Number": "559-278-4373",
		"Email Address": "ldennis@csufresno.edu",
		"Office Location": "Science II 255",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "William Kerney",
		"Faculty Role": "Part Time Faculty",
		"Desk Phone Number": "559-278-4373",
		"Department Phone Number": "559-278-4373",
		"Email Address": "wkerney@mail.fresnostate.edu",
		"Office Location": "Science II 255",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Dr. Ming Li",
		"Faculty Role": "Faculty",
		"Desk Phone Number": "559-278-4792",
		"Department Phone Number": "559-278-4373",
		"Email Address": "mingli@csufresno.edu",
		"Office Location": "Science II 272",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Alex Liu",
		"Faculty Role": "Faculty / Chair",
		"Desk Phone Number": "559-278-4789",
		"Department Phone Number": "559-278-4373",
		"Email Address": "shliu@csufresno.edu",
		"Office Location": "Science II 271",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Athanasios Aris (Thanos) Panagopoulos",
		"Faculty Role": "Faculty",
		"Desk Phone Number": "559-278-5152",
		"Department Phone Number": "559-278-4373",
		"Email Address": "apanagopoulos@csufresno.edu",
		"Office Location": "Science II 248",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Jin Park",
		"Faculty Role": "Faculty",
		"Desk Phone Number": "559-278-4307",
		"Department Phone Number": "559-278-4373",
		"Email Address": "jpark@csufresno.edu",
		"Office Location": "Science II 277",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Matin Pirouz Nia",
		"Faculty Role": "Faculty",
		"Desk Phone Number": "559-278-5142",
		"Department Phone Number": "559-278-4373",
		"Email Address": "mpirouz@csufresno.edu",
		"Office Location": "Science II 246",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "David Ruby",
		"Faculty Role": "Part Time Faculty",
		"Desk Phone Number": "559-278-4312",
		"Department Phone Number": "559-278-4373",
		"Email Address": "druby@csufresno.edu",
		"Office Location": "Science II 273",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Todd Wilson",
		"Faculty Role": "Faculty",
		"Desk Phone Number": "559-278-9138",
		"Department Phone Number": "559-278-4373",
		"Email Address": "twilson@csufresno.edu",
		"Office Location": "Science II 250",
		"Department": "Computer Science Department",
		"Mail Stop": "ST109"
	},
	{
		"Name": "Max J Tsai",
		"Faculty Role": "Part Time Faculty / Interim Director of Digital Transformation",
		"Desk Phone Number": "559-278-4180",
		"Department Phone Number": "559-278-4180",
		"Email Address": "janq@csufresno.edu",
		"Office Location": "IT 300",
		"Department": "Computer Science Department",
		"Mail Stop": "N/A"
	}
];

//Creating Arrays to hold all the data
let nameProfArr = [];
let roles = [];
let deskPhone = [];
let departmentPhone = [];
let emailAdd = [];
let officeLoc = [];
let department = [];
let mailStop = [];

for(let i = 0; i < profData.length; i++){
	nameProfArr.push(profData[i]["Name"]);
	roles.push(profData[i]["Faculty Role"]);
	deskPhone.push(profData[i]["Desk Phone Number"]);
	departmentPhone.push(profData[i]["Department Phone Number"]);
    emailAdd.push(profData[i]["Email Address"]);
	officeLoc.push(profData[i]["Office Location"]);
	department.push(profData[i]["Department"]);
	mailStop.push(profData[i]["Mail Stop"]);
};

function showProfData(){
	let docBod = document.getElementById("ProfessorContainer");
	let newTable= document.createElement('TABLE');
	let newTableBod = document.createElement('TBODY');
	docBod.appendChild(newTable);
	newTable.appendChild(newTableBod);

    for(let i = 0; i < profData.length; i++){
		let tr = document.createElement('TR');
		newTableBod.appendChild(tr);
		for(let j = 0; j < 4 && i < profData.length; j++, i++){
			if(profData[i]["Name"] == ""){
			}
			else{
				let profInfo = createProfTd(nameProfArr[i], roles[i], deskPhone[i], 
                                         departmentPhone[i], emailAdd[i], officeLoc[i], 
                                         department[i], mailStop[i]);
				tr.appendChild(profInfo);
			}
			if(j == 3)
				i--;
        }
        
    }
}

function createProfTd(name, role, desk, departP, em, officeL, dep, mail){

	let profDataList = document.createElement('TD')

	let profName = document.createElement('H2');
	profName.appendChild(document.createTextNode(name));

	let roleName = document.createElement('P');
	roleName.appendChild(document.createTextNode(role));

    let deskP = document.createElement('P');
	deskP.appendChild(document.createTextNode(desk));

    let departPh = document.createElement('P');
	departPh.appendChild(document.createTextNode(departP));

    let email = document.createElement('P');
	email.appendChild(document.createTextNode(em));
    
    let office = document.createElement('P');
	office.appendChild(document.createTextNode(officeL));

    let depar = document.createElement('P');
	depar.appendChild(document.createTextNode(dep));

    let mailS = document.createElement('P');
	mailS.appendChild(document.createTextNode(mail));

	profDataList.appendChild(profName);
	profDataList.appendChild(roleName);
	profDataList.appendChild(deskP);
	profDataList.appendChild(departPh);
    profDataList.appendChild(email);
    profDataList.appendChild(office);
    profDataList.appendChild(depar);
    profDataList.appendChild(mailS);
	return profDataList;
}


function getProfData(){	//Get all professor data
	return profData;
}

//Return a single instance of the professor
function getProfName(index){
	return nameProfArr[index];
}
function getProfRole(index){
	return roles[index];
}
function getProfDeskPhone(index){
	return deskPhone[index];
}
function getProfDepartMentPhone(index){
	return departmentPhone[index];
}
function getProfEmail(index){
	return emailAdd[index];
}
function getProfOfficeLoc(index){
	return officeLoc[index];
}
function getProfDepartment(index){
	return department[index];
}
function getProfMailStop(index){
	return mailStop[index];
}

//Return all info on a professor
function getAllDataOfOneProf(index){
	let singleProfData = [getProfName(index), getProfRole(index), getProfDeskPhone(index), getProfDepartMentPhone(index),
						  getProfEmail(index), getProfOfficeLoc(index), getProfDepartment(index), getProfMailStop(index)]
	return singleProfData;
}







//This is where the side bar code is
//Left side bar
function openSideNav(){
	document.getElementById("sideBar").style.width = "400px";	//These are basically moving the sidebars into the page
	document.getElementById("ClassContainer").style.marginLeft  = "400px";
	document.getElementById("ProfessorContainer").style.marginLeft  = "400px";
	document.getElementById("sideBarClickMe").style.marginLeft  = "400px";
	document.getElementById("sideBarClickMeTwo").style.marginLeft = "400px";
}

function closeSideBar(){
	document.getElementById("sideBar").style.width = "0";	  //These will move the sidebars outside the page
	document.getElementById("ClassContainer").style.marginLeft  = "0";
	document.getElementById("ProfessorContainer").style.marginLeft  = "0";
	document.getElementById("sideBarClickMe").style.marginLeft  = "0";
	document.getElementById("sideBarClickMeTwo").style.marginLeft = "0";
}

//Right side bar
function openSideNavTwo(){
	document.getElementById("sideBarTwo").style.width = "400px";
	document.getElementById("ClassContainer").style.marginRight  = "400px";
	document.getElementById("ProfessorContainer").style.marginRight  = "400px";
	document.getElementById("sideBarClickMe").style.marginRight  = "400px";
	document.getElementById("sideBarClickMeTwo").style.marginRight = "400px";
}

function closeSideBarTwo(){
	document.getElementById("sideBarTwo").style.width = "0";
	document.getElementById("ClassContainer").style.marginRight  = "0";
	document.getElementById("ProfessorContainer").style.marginRight  = "0";
	document.getElementById("sideBarClickMe").style.marginRight  = "0";
	document.getElementById("sideBarClickMeTwo").style.marginRight = "0";
}







//This is the auto complete search bar functions. It takes an array and will help auto fill the info based on input
function autocomplete(inp, arr) {
	/*the autocomplete function takes two arguments,
	the text field element and an array of possible autocompleted values:*/
	var currentFocus;
	/*execute a function when someone writes in the text field:*/
	inp.addEventListener("input", function(e) {
		var a, b, i, val = this.value;
		/*close any already open lists of autocompleted values*/
		closeAllLists();
		if (!val) { return false;}
		currentFocus = -1;
		/*create a DIV element that will contain the items (values):*/
		a = document.createElement("DIV");
		a.setAttribute("id", this.id + "autocomplete-list");
		a.setAttribute("class", "autocomplete-items");
		/*append the DIV element as a child of the autocomplete container:*/
		this.parentNode.appendChild(a);
		/*for each item in the array...*/
		for (i = 0; i < arr.length; i++) {
		  /*check if the item starts with the same letters as the text field value:*/
		  if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
			/*create a DIV element for each matching element:*/
			b = document.createElement("DIV");
			/*make the matching letters bold:*/
			b.innerHTML = "<strong>" + arr[i].substr(0, val.length) + "</strong>";
			b.innerHTML += arr[i].substr(val.length);
			/*insert a input field that will hold the current array item's value:*/
			b.innerHTML += "<input type='hidden' value='" + arr[i] + "'>";
			/*execute a function when someone clicks on the item value (DIV element):*/
				b.addEventListener("click", function(e) {
				/*insert the value for the autocomplete text field:*/
				inp.value = this.getElementsByTagName("input")[0].value;
				/*close the list of autocompleted values,
				(or any other open lists of autocompleted values:*/
				closeAllLists();
			});
			a.appendChild(b);
		  }
		}
	});

	/*execute a function presses a key on the keyboard:*/
	inp.addEventListener("keydown", function(e) {
		var x = document.getElementById(this.id + "autocomplete-list");
		if (x) x = x.getElementsByTagName("div");
		if (e.keyCode == 40) {
		  /*If the arrow DOWN key is pressed,
		  increase the currentFocus variable:*/
		  currentFocus++;
		  /*and and make the current item more visible:*/
		  addActive(x);
		} else if (e.keyCode == 38) { //up
		  /*If the arrow UP key is pressed,
		  decrease the currentFocus variable:*/
		  currentFocus--;
		  /*and and make the current item more visible:*/
		  addActive(x);
		} else if (e.keyCode == 13) {
		  /*If the ENTER key is pressed, prevent the form from being submitted,*/
		  e.preventDefault();
		  if (currentFocus > -1) {
			/*and simulate a click on the "active" item:*/
			if (x) x[currentFocus].click();
		  }
		}
	});

	function addActive(x) {
	  /*a function to classify an item as "active":*/
	  if (!x) return false;
	  /*start by removing the "active" class on all items:*/
	  removeActive(x);
	  if (currentFocus >= x.length) currentFocus = 0;
	  if (currentFocus < 0) currentFocus = (x.length - 1);
	  /*add class "autocomplete-active":*/
	  x[currentFocus].classList.add("autocomplete-active");
	}
	function removeActive(x) {
	  /*a function to remove the "active" class from all autocomplete items:*/
	  for (var i = 0; i < x.length; i++) {
		x[i].classList.remove("autocomplete-active");
	  }
	}
	function closeAllLists(elmnt) {
	  /*close all autocomplete lists in the document,
	  except the one passed as an argument:*/
	  var x = document.getElementsByClassName("autocomplete-items");
	  for (var i = 0; i < x.length; i++) {
		if (elmnt != x[i] && elmnt != inp) {
		x[i].parentNode.removeChild(x[i]);
	  }
	}
  }
  /*execute a function when someone clicks somewhere else in the document:*/
  document.addEventListener("click", function (e) {
	  closeAllLists(e.target);
  });
  }

  let csciNamesAndPages = [
	{
		"Names": "CSCI 1. Critical Thinking and Computer Science",
		"pageName": "csciOnePage.html"
	},
	{
		"Names": "CSCI 5. Computer and Applications",
		"pageName": "csciFivePage.html"
	},
	{
		"Names": "CSCI 30. Introduction to the Internet",
		"pageName": "csciThirdyPage.html"
	},
	{
		"Names": "CSCI 39. Computer Science Principles",
		"pageName": "csciThirdyNinePage.html"
	},
	{
		"Names": "CSCI 40. Introduction to Programming and Problem Solving",
		"pageName": "csciFourtyPage.html"
	},
	{
		"Names": "CSCI 41. Introduction to Data Structures",
		"pageName": "csciFourtyOnePage.html"
	},
	{
		"Names": "CSCI 60. Foundations of Computer Science",
		"pageName": ""
	},
	{
		"Names": "CSCI 68. Computing Practice and Programming",
		"pageName": ""
	},
	{
		"Names": "CSCI 100. Introduction to Computational Science",
		"pageName": ""
	},
	{
		"Names": "CSCI 101. Computational Foundations for Bioinformatics",
		"pageName": ""
	},
	{
		"Names": "CSCI 108. Algorithmic Thinking",
		"pageName": ""
	},
	{
		"Names": "CSCI 112. Introduction to Computer Systems",
		"pageName": ""
	},
	{
		"Names": "CSCI 113. Introduction to Computer Organization",
		"pageName": ""
	},
	{
		"Names": "CSCI 114. Introduction to Operating Systems",
		"pageName": ""
	},
	{
		"Names": "CSCI 115. Algorithms and Data Structures",
		"pageName": ""
	},
	{
		"Names": "CSCI 117. Structures of Programming Languages",
		"pageName": ""
	},
	{
		"Names": "CSCI 119. Introduction to Finite Automata",
		"pageName": ""
	},
	{
		"Names": "CSCI 124. Introduction to File Processing",
		"pageName": ""
	},
	{
		"Names": "CSCI 126. Database Systems",
		"pageName": ""
	},
	{
		"Names": "CSCI 130. Web Programming",
		"pageName": ""
	},
	{
		"Names": "CSCI 134. Compiler Design",
		"pageName": ""
	},
	{
		"Names": "CSCI 146. Systems Architecture",
		"pageName": ""
	},
	{
		"Names": "CSCI 148. Systems Programming",
		"pageName": ""
	},
	{
		"Names": "CSCI 150. Introduction to Software Engineering",
		"pageName": ""
	},
	{
		"Names": "CSCI 152E. Software Engineering",
		"pageName": ""
	},
	{
		"Names": "CSCI 154. Simulation",
		"pageName": ""
	},
	{
		"Names": "CSCI 156. Internetworking Systems and Protocols",
		"pageName": ""
	},
	{
		"Names": "CSCI 164. Artificial Intelligence Programming",
		"pageName": ""
	},
	{
		"Names": "CSCI 166. Principles of Artificial Intelligence",
		"pageName": ""
	},
	{
		"Names": "CSCI 168E. Big Data Analytics",
		"pageName": ""
	},
	{
		"Names": "CSCI 172. Computer Graphics",
		"pageName": ""
	},
	{
		"Names": "CSCI 173. Advanced Computer Graphics",
		"pageName": ""
	},
	{
		"Names": "CSCI 174. Design and Analysis of Algorithms",
		"pageName": ""
	},
	{
		"Names": "CSCI 176. Parallel Processing",
		"pageName": ""
	},
	{
		"Names": "CSCI 177. Distributed Computer Systems",
		"pageName": ""
	},
	{
		"Names": "CSCI 186. Formal Languages and Automata",
		"pageName": ""
	},
	{
		"Names": "CSCI 188. Introduction to Computability",
		"pageName": ""
	},
	{
		"Names": "CSCI 190. Independent Study",
		"pageName": ""
	},
	{
		"Names": "CSCI 191T. Topics in Computer Science",
		"pageName": ""
	},
	{
		"Names": "CSCI 191T. Computer Security",
		"pageName": ""
	},
	{
		"Names": "CSCI 194I. Cooperative Education",
		"pageName": ""
	},
	{
		"Names": "CSCI 198. Project",
		"pageName": ""
	},
	{
		"Names": "CSCI 200. Introduction to Research in Computer Science",
		"pageName": ""
	},
	{
		"Names": "CSCI 201. CSCI Colloquium",
		"pageName": ""
	},
	{
		"Names": "CSCI 213. Computer Organization",
		"pageName": ""
	},
	{
		"Names": "CSCI 217. Programming Language Principles",
		"pageName": ""
	},
	{
		"Names": "CSCI 226. Advanced Database Systems",
		"pageName": ""
	},
	{
		"Names": "CSCI 230. Advanced Web Application Development",
		"pageName": ""
	},
	{
		"Names": "CSCI 244. Operating Systems",
		"pageName": ""
	},
	{
		"Names": "CSCI 246. Computer Architecture",
		"pageName": ""
	},
	{
		"Names": "CSCI 250. Advanced Software Engineering",
		"pageName": ""
	},
	{
		"Names": "CSCI 252. Software Development and Environments",
		"pageName": ""
	},
	{
		"Names": "CSCI 253. Human-Computer Interaction",
		"pageName": ""
	},
	{
		"Names": "CSCI 256. Wireless Communications and Mobile Computing",
		"pageName": ""
	},
	{
		"Names": "CSCI 264. Artificial Intelligence",
		"pageName": ""
	},
	{
		"Names": "CSCI 272. Computer Graphics",
		"pageName": ""
	},
	{
		"Names": "CSCI 274. Combinatorial Algorithms",
		"pageName": ""
	},
	{
		"Names": "CSCI 282. Theory of Computation",
		"pageName": ""
	},
	{
		"Names": "CSCI 284. Automata Theory",
		"pageName": ""
	},
	{
		"Names": "CSCI 290. Independent Study",
		"pageName": ""
	},
	{
		"Names": "CSCI 291T. Seminar",
		"pageName": ""
	},
	{
		"Names": "CSCI 291T. Introduction to Reinforcement Learning",
		"pageName": ""
	},
	{
		"Names": "CSCI 291T. Image Processing",
		"pageName": ""
	},
	{
		"Names": "CSCI 297. Grad Synthesis",
		"pageName": ""
	},
	{
		"Names": "CSCI 298. Research Project",
		"pageName": ""
	},
	{
		"Names": "CSCI 298C. Project Continuation",
		"pageName": ""
	},
	{
		"Names": "CSCI 299. Master's Thesis",
		"pageName": ""
	},
	{
		"Names": "CSCI 299C. Thesis Continuation",
		"pageName": ""
	}
]

let csciProfNamesAndPages = [
	{
		"Names": "Dhanyu Eshaka Amarasinghe",
		"pageName": "facultyDhanyuEshakaAmarasinghe.html"
	},
	{
		"Names": "Dr. Brent Auernheimer",
		"pageName": "facultyBrentAuernheimer.html"
	},
	{
		"Names": "Santanu Banerjee",
		"pageName": "facultySantanuBanerjee.html"
	},
	{
		"Names": "Amith Kamath Belman",
		"pageName": "facultyAmith Kamath Belman.html"
	},
	{
		"Names": "Hubert Cecotti",
		"pageName": "facultyHubertCecotti.html"
	},
	{
		"Names": "Lisa Dennis",
		"pageName": "facultyLisaDennis.html"
	},
	{
		"Names": "William Kerney",
		"pageName": "facultyWilliamKerney.html"
	},
	{
		"Names": "Dr. Ming Li",
		"pageName": "facultyMingLi.html"
	},
	{
		"Names": "Alex Liu",
		"pageName": "Alex Liu"
	},
	{
		"Names": "Athanasios Aris (Thanos) Panagopoulos",
		"pageName": "Athanasios Aris (Thanos) Panagopoulos"
	},
	{
		"Names": "Jin Park",
		"pageName": "Jin Park"
	},
	{
		"Names": "Matin Pirouz Nia",
		"pageName": "Matin Pirouz Nia"
	},
	{
		"Names": "David Ruby",
		"pageName": "David Ruby"
	},
	{
		"Names": "Todd Wilson",
		"pageName": "Todd Wilson"
	}
]
  function goHere(){
	let getSearch = document.getElementById("searchBar").value;
	let i = 0;
	while(i < csciNamesAndPages.length){
		if(getSearch == csciNamesAndPages[i]["Names"]){
			window.open(csciNamesAndPages[i]["pageName"]);
		}
		i++;
		if(i == csciNamesAndPages.length){
			window.open("errorPage.html");
		}
	}
  }

  function goHereTwo(){
	let getSearch = document.getElementById("searchBarTwo").value;
	let i = 0;
	while(i < csciProfNamesAndPages.length){
		if(getSearch == csciProfNamesAndPages[i]["Names"]){
			window.open(csciProfNamesAndPages[i]["pageName"]);
		}
		i++;
		if(i == csciProfNamesAndPages.length){
			window.open("errorPage.html");
		}
	}
  }